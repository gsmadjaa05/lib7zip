// TestExtractDirPassword.cpp : Extract password-protected archive to directory
//
...
{
	if (argc < 4) {
		wprintf(L"Usage: TestExtractDirPassword [archive file] [output directory] [password]\n");
		wprintf(L"Example: TestExtractDirPassword Test7ZipCryptFileName.7z extracted_files hello\n");
		return 1;
	}
	...
	std::string passwordStr = argv[3];
	wstring password = std::wstring(passwordStr.begin(), passwordStr.end());
	...
	if (lib.OpenArchive(&stream, &pArchive, password)) {
		...
		// Set archive password for extraction
		pArchive->SetArchivePassword(password);
		...
		for(unsigned int i = 0; i < numItems; i++) {
			...
			if (pArchive->GetItemInfo(i, &pArchiveItem)) {
				...
				bool isEncrypted = pArchiveItem->IsEncrypted();

				wprintf(L"Item %d: %ls (isDir: %d, encrypted: %d)\n", i, fullPath.c_str(), isDir, isEncrypted);
		...
		wprintf(L"open archive %s with password fail, LastError=%X\n", archivePath.c_str(), lib.GetLastError());
	}
